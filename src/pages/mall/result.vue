<template>
  <div class="mallResult">
    ...
    <div class="mallFilter">
      ...
      <a-row>
        <a-col :span="1"><div class="label"> 排序: </div></a-col>
        <a-col :span="23">
          <a-radio-group defaultValue="learned_total" v-model="filters.order_by_column" @change="sort">
            <a-radio-button value="learned_total" @click="radioButton('learned_total')">
              学习人数
              <span v-if="filters.order_by_column === 'learned_total'">
                <a-icon type="arrow-up" v-if="filters.order_by_direction === 'asc'" />
                <a-icon type="arrow-down" v-if="filters.order_by_direction === 'desc'" />
              </span>
            </a-radio-button>
            <a-radio-button value="published_time" @click="radioButton('published_time')">
              上架时间
              <span v-if="filters.order_by_column === 'published_time'">
                <a-icon type="arrow-up" v-if="filters.order_by_direction === 'asc'" />
                <a-icon type="arrow-down" v-if="filters.order_by_direction === 'desc'" />
              </span>
            </a-radio-button>
            <a-radio-button value="uniform_price" @click="radioButton('uniform_price')">
              价格
              <span v-if="filters.order_by_column === 'uniform_price'">
                <a-icon type="arrow-up" v-if="filters.order_by_direction === 'asc'" />
                <a-icon type="arrow-down" v-if="filters.order_by_direction === 'desc'" />
              </span>
            </a-radio-button>
          </a-radio-group>
          <a-checkbox @change="checkIsFree" v-model="filters.is_free" class="freeCheck">只看免费</a-checkbox>
        </a-col>
      </a-row>
    </div>
            ...
            <a-card hoverable style="width: 100%;position:relative" @click="getDetail(item)">
              ...
              <a-card-meta>
                ...
                <template slot="description">
                  ...
                  <a-row>
                    <a-col :span="6" class="price" v-if="item.uniform_price === 0"> <strong> 免费 </strong> </a-col>
                    <a-col :span="6" class="price" v-else><span>￥</span>{{(item.uniform_price / 100).toFixed(2)}}</a-col>
  ...
  methods: {
    getList (current_page = this.pagination.current_page) {
      ...
      this.$api.mall.list({
        ...
        is_free: 0 + filters.is_free,
        order_by_column: filters.order_by_column,
